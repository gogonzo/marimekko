...
[![CRAN
downloads](https://cranlogs.r-pkg.org/badges/marimekko)](https://cran.r-project.org/package=marimekko)

marimekko (mosaic) plots for **ggplot2**.

A one-sided formula controls the variable hierarchy and split
directions. Column widths and segment heights encode marginal and
conditional proportions of categorical variables.
...
|--------------------------------|----------------------------------------------------------------------|
| Core marimekko plot            | [`geom_marimekko()`](reference/geom_marimekko.md)                    |
| Text labels on tiles           | [`geom_marimekko_text()`](reference/geom_marimekko_text.md)          |
| Labels with background box     | [`geom_marimekko_label()`](reference/geom_marimekko_label.md)        |
| Marginal percentages on x-axis | `show_percentages = TRUE`                                            |
| Compute tiles without plotting | [`fortify_marimekko()`](reference/fortify_marimekko.md)              |
| Minimal mosaic theme           | [`theme_marimekko()`](reference/theme_marimekko.md)                  |
...
| Plotly interactivity           | [`plotly::ggplotly()`](https://rdrr.io/pkg/plotly/man/ggplotly.html) |
...
- **`StatMarimekko`** parses the formula, recursively partitions the
  ...
  `ymax`) with computed variables (`.residuals`, `.proportion`,
  `.marginal`).
- Tiles are rendered via **`GeomRect`** with sensible defaults (white
  borders, slight transparency).
- Axis labels are automatically placed by the geom at tile midpoints.

## Why to use marimekko?

`marimekko` was designed to avoid pain points in other existing
packages.

- **Minimal dependencies** – `ggplot2` as only dependency
- **No internal ggplot2 API usage** – won’t break on ggplot2 updates
- **Easily extendable** – `StatMarimekkoTiles` exposes tile data so you
  can pair it with any ggplot2 geom to build custom companion layers
  (bubbles, residual markers, etc.)
- **Formula-based API** – `~ a | b | c` encodes both variables and
  directions
- **Works without [`library()`](https://rdrr.io/r/base/library.html)** –
  [`marimekko::geom_marimekko()`](reference/geom_marimekko.md) just
  works
- **Respects factor levels** – user-set
  [`levels()`](https://rdrr.io/r/base/levels.html) are honored
- **In-formula expressions** – `~ factor(cyl) | cut(mpg, breaks = 3)`
  works
- **Plotly compatible** –
  [`ggplotly()`](https://rdrr.io/pkg/plotly/man/ggplotly.html) works out
  of the box
