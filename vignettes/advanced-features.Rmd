...
ggplot(ucb_a) +
  ...
  geom_marimekko_jitter(seed = 42, size = 0.8) +
...
and `geom_marimekko_jitter()` automatically read tile positions from a
...
tiles$significant <- abs(tiles$.resid) > 2
...
ggplot(titanic) +
  ...
  geom_label(
    ...
    aes(x = x, y = y, label = paste0("r=", round(.resid, 1))),
...
`marimekko` ships with an earthy, Nordic-inspired colour palette and
companion scale functions. Use `scale_fill_marimekko()` and
`scale_colour_marimekko()` for a consistent look:
